...
describe('NotificationService', () => {
    ...
    beforeEach(() => {
        ...
        jest.clearAllMocks();
        // モックの設定
        mockSqliteService = sqliteService_1.SQLiteService.getInstance();
        mockAuditLogService = auditLogService_1.AuditLogService.getInstance();
        // WebSocketのモック
        mockWebSocket = {
            send: jest.fn(),
            on: jest.fn(),
        ...
        ws_1.default.Server.mockImplementation(() => mockWebSocketServer);
        // NotificationServiceのインスタンス化
        notificationService = notificationService_1.NotificationService.getInstance();
    });
    afterEach(() => {
        // シングルトンをリセット
        notificationService_1.NotificationService['instance'] = undefined;
    });
    describe('initialize', () => {
        it('should initialize WebSocket server and set up event listeners', async () => {
            ...
            expect(ws_1.default.Server).toHaveBeenCalledWith({
                noServer: true
            });
    ...
    describe('handleUpgrade', () => {
        it('should authenticate user and upgrade connection', async () => {
            ...
            // アップグレードハンドラを呼び出す
            await notificationService.handleUpgrade(mockRequest, mockSocket, mockHead);
            // トークンが検証されたことを確認
            expect(mockVerifyJwt).toHaveBeenCalledWith('valid-token');
            // WebSocketサーバーがアップグレードイベントを発行したことを確認
            expect(mockWebSocketServer.emit).toHaveBeenCalledWith('connection', expect.any(Object), { userEmail: 'test@example.com' });
        });
        it('should reject connection with invalid token', async () => {
            ...
            // 接続が拒否されたことを確認
            expect(mockSocket.write).toHaveBeenCalledWith(expect.stringContaining('HTTP/1.1 401 Unauthorized'));
