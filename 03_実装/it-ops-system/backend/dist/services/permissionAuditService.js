...
exports.PermissionAuditService = void 0;
...
class PermissionAuditService {
    ...
    async recordChange(record) {
        try {
            const result = await this.sqlite.run(`INSERT INTO permission_audit (
     ...
     * @param record 監査レコード
     */
    async notifyPermissionChange(record) {
        try {
            // グローバル管理者に通知
    ...
    async getGlobalAdmins() {
        try {
            return await this.sqlite.all(`SELECT id, email FROM users WHERE is_global_admin = 1`);
        }
        catch (error) {
            logger.logError(error, {
                context: 'PermissionAuditService',
     ...
     * @returns 監査レコードのリスト
     */
    async searchAuditRecords(filter) {
    ...
    async getAuditRecordById(id) {
        try {
            const row = await this.sqlite.get(`SELECT * FROM permission_audit WHERE id = ?`, [id]);
            if (!row) {
                return null;
            }
            return {
     ...
     * @returns 統計情報
     */
    async getChangeStatistics(filter) {
        try {
            const params = [];
            let whereClause = '1=1';
            if (filter.startDate) {
     ...
     * レビューデータのバリデーション
     */
    validateReviewData(recordId, comments) {
     ...
     * @param comments コメント
     */
    async recordReview(recordId, reviewerId, reviewerEmail, approved, comments) {
        try {
            // レビューデータのバリデーション
            ...
            await this.sqlite.run(`INSERT INTO permission_audit_reviews (
     ...
     * レビュー結果の通知
     */
    async notifyReviewResult(record, review) {
        try {
            // 変更者への通知
            await this.notificationService.sendNotification({
     ...
     * @returns 権限変更履歴
     */
    async getUserPermissionHistory(userEmail, limit = 100) {
        try {
            const rows = await this.sqlite.all(`SELECT * FROM permission_audit 
        WHERE target_email = ? 
        ORDER BY timestamp DESC 
        LIMIT ?`, [userEmail, limit]);
            return rows.map(row => ({
                id: typeof row.id === 'string' ? Number(row.id) : row.id,
                timestamp: new Date(row.timestamp),
                actorId: row.actor_id,
...
exports.PermissionAuditService = PermissionAuditService;
//# sourceMappingURL=permissionAuditService.js.map