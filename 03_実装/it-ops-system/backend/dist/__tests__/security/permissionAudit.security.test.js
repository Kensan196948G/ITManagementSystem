...
describe('権限変更監査セキュリティテスト', () => {
    ...
    describe('権限チェック', () => {
        it('権限のないユーザーによる監査記録の取得を拒否', async () => {
            ...
            await expect(auditService.searchAuditRecords({}))
                .rejects
                .toThrow(AuditError_1.AuditPermissionError);
        });
        it('権限のないユーザーによるレビュー操作を拒否', async () => {
            jest.spyOn(authService, 'hasPermission').mockResolvedValue(false);
            await expect(auditService.recordReview(1, 'user123', 'user@example.com', true, 'Test comment')).rejects.toThrow(AuditError_1.AuditPermissionError);
        });
    });
    ...
    describe('監査ログのセキュリティ', () => {
        ...
        it('監査ログの改ざんを検知', async () => {
            ...
            await sqlite.run('UPDATE permission_audit SET action = ? WHERE id = ?', ['modified', recordId]);
    ...
    describe('データアクセス制御', () => {
        it('ユーザーは自分に関連する監査ログのみアクセス可能', async () => {
            ...
            records.forEach(record => {
                expect(record.actorEmail === 'user@example.com' ||
                    record.targetEmail === 'user@example.com').toBe(true);
            });
        });
    });
    describe('セキュリティヘッダー', () => {
...
//# sourceMappingURL=permissionAudit.security.test.js.map