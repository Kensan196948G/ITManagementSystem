...
describe('AuditMetricsService', () => {
  let metricsService: AuditMetricsService;
  ...
  beforeEach(async () => {
    ...
    metricsService = AuditMetricsService.getInstance();
  });
  ...
  describe('recordMetric', () => {
    ...
    it('ラベルなしでメトリクスを記録できること', async () => {
      ...
      expect(mockSQLite.run).toHaveBeenCalledWith(
        expect.any(String),
        expect.arrayContaining([expect.any(String), 1.0, '{}'])
      );
    });
  ...
  describe('getAggregatedMetrics', () => {
    ...
    it('ラベルフィルターで集計できること', async () => {
      ...
      expect(mockSQLite.all).toHaveBeenCalledWith(
        expect.stringContaining('metric_labels LIKE'),
        expect.arrayContaining(['%"label1":"value1"%'])
      );
    });
  });
});