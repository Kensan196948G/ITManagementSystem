...
describe('ストレージレイヤーの統合テスト', () => {
  ...
  beforeEach(async () => {
    ...
    sqlite = SQLiteService.getInstance();
  ...
  describe('データの永続化', () => {
    it('SQLiteに正常にデータを保存できること', async () => {
      const result = await sqlite.run(
        'INSERT INTO permission_audit (actor_id, action) VALUES (?, ?)',
        ['user1', 'test']
      );
      expect(result.lastID).toBeGreaterThan(0);

      const record = await sqlite.get(
        'SELECT * FROM permission_audit WHERE actor_id = ?',
        ['user1']
      );
    ...
    it('一意性制約が機能すること', async () => {
      await sqlite.run(
        'INSERT INTO audit_reviews (record_id, reviewer_id) VALUES (?, ?)',
        [1, 'reviewer1']
      );

      await expect(sqlite.run(
        'INSERT INTO audit_reviews (record_id, reviewer_id) VALUES (?, ?)',
        [1, 'reviewer1']
      )).rejects.toThrow();
    });
  ...
  describe('エラー処理', () => {
    it('SQLite接続エラーを適切に処理すること', async () => {
      const invalidSqlite = new SQLiteService(':memory:');
  ...
  describe('パフォーマンス最適化', () => {
    it('インデックスが正しく使用されること', async () => {
      ...
      for (let i = 0; i < 1000; i++) {
        await sqlite.run(
          'INSERT INTO permission_audit (actor_id, action, timestamp) VALUES (?, ?, ?)',
          [`user${i}`, 'test', Date.now()]
        );
      }

      const startTime = Date.now();
      await sqlite.all(
        'SELECT * FROM permission_audit WHERE timestamp > ? ORDER BY timestamp DESC LIMIT 10',
        [Date.now() - 86400000]
      );
