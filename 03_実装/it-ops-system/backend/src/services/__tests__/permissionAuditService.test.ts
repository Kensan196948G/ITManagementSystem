...
describe('PermissionAuditService', () => {
  ...
  beforeEach(async () => {
    ...
    permissionAuditService = PermissionAuditService.getInstance();
  });
  ...
  describe('recordReview', () => {
    it('権限変更レビューを正常に記録できること', async () => {
      await permissionAuditService.recordReview(
        1,
        'reviewer123',
    ...
    it('エラー時に例外をスローすること', async () => {
      ...
      await expect(
        permissionAuditService.recordReview(
          1,
          'reviewer123',
          'reviewer@example.com',
          true,
  ...
  describe('notifyPermissionChange', () => {
    ...
    beforeEach(() => {
      ...
      (NotificationService as any).getInstance = jest.fn().mockReturnValue(mockNotificationService);
    });
    ...
    it('自分自身の変更の場合、対象ユーザーに通知が送信されないこと', async () => {
      ...
      expect(mockNotificationService.sendNotification).not.toHaveBeenCalledWith(
        expect.objectContaining({
          userId: testRecord.targetId,
          userEmail: testRecord.targetEmail
        })
