...
import { requireGlobalAdmin } from '../middleware/authorization';
import { verifyToken } from '../routes/auth';
import GraphPermissionService from '../services/graphPermissionService';
 ...
 */
router.get('/users/:userEmail', verifyToken, async (req, res) => {
  try {
 ...
 * グローバル管理者のみ実行可能
 */
router.post('/users/:userEmail/grant', verifyToken, requireGlobalAdmin, async (req, res) => {
  try {
    const { userEmail } = req.params;
    ...
    const result = await graphPermissionService.grantPermission(
      userEmail,
      permission,
      scope,
      operatorEmail
 ...
 * グローバル管理者のみ実行可能
 */
router.post('/users/:userEmail/revoke', verifyToken, requireGlobalAdmin, async (req, res) => {
  try {
    const { userEmail } = req.params;
    const { permission, scope } = req.body;
    const operatorEmail = req.user?.email || 'system';
    ...
    const result = await graphPermissionService.revokePermission(
      userEmail,
      permission,
      scope,
      operatorEmail
 ...
 * 一般ユーザーも閲覧可能
 */
router.get('/operations-summary', verifyToken, async (_req, res) => {
  try {
