 ...
 */
export class DoubleAuthorization {
  /**
   * グローバル管理者権限を要求
   * フロントエンドでのチェックに加えて、バックエンドでも再検証
   */
  static requireGlobalAdmin(req: Request, res: Response, next: NextFunction) {
    try {
      // トークンが検証済みであることを確認
   ...
   */
  static requireSelfOrAdmin(userIdParam: string = 'userEmail') {
    return async (req: Request, res: Response, next: NextFunction) => {
      try {
        // トークンが検証済みであることを確認
        if (!req.user || !req.user.email) {
   ...
   */
  static restrictIpAccess(allowedIps: string[]) {
